...
class SS_Mngmt_Env(Env):
    ...
    def __init__(
        ...
        self.final_echelon_nodes = [
            node
            for node in self.graph.nodes
            if node not in ["S", "D"]
            and any(successor == "D" for successor in self.graph.successors(node))
        ]

        self.internal_nodes = [
            node for node in self.graph.nodes if node not in ["S", "D"]
        ]
        num_nodes = len(self.internal_nodes)
        ...
        action_choices = np.full(num_nodes, n_actions)
        ...
        # Initialize current_demand as zeros for all nodes
        current_demand = np.zeros(num_nodes, dtype=np.float32)

        # Fill in demands only for the final echelon nodes
        for i, node in enumerate(self.final_echelon_nodes):
            node_idx = self.node_to_index(node)
            current_demand[node_idx] = self.actual_demands[0, i]

        # Compute internal demand for upstream nodes based on downstream orders
        for node in self.internal_nodes:
            if node not in self.final_echelon_nodes:
                node_idx = self.node_to_index(node)
                node_demand = 0
                current_demand[node_idx] = node_demand
    ...
    def step(self, action):
        ...
        # Compute the new orders from the action
        self.new_order = [self.order_quantities[a] for a in action]

        # Initialize current demands to zero
        current_demand = np.zeros_like(self.inventory, dtype=np.float32)

        # Assign demands to final echelon nodes from actual_demands
        # self.actual_demands now correspond only to final echelon nodes
        for i, node in enumerate(self.final_echelon_nodes):
            node_idx = self.node_to_index(node)
            current_demand[node_idx] = self.actual_demands[timestep, i]

        # For upstream nodes, compute demand as sum of orders of downstream nodes
        for node in self.graph.nodes:
            if node not in ["S", "D"] and node not in self.final_echelon_nodes:
                node_idx = self.node_to_index(node)
                successors = list(self.graph.successors(node))
                node_demand = 0
                for succ in successors:
                    succ_idx = self.node_to_index(succ)
                    # Demand at this node = sum of the orders placed by downstream successors
                    node_demand += self.new_order[succ_idx]
                current_demand[node_idx] = node_demand
    ...
    def node_to_index(self, node):
        ...
        """
        return self.internal_nodes.index(node)
    ...
    def planned_demand(self, demand_mean=10, demand_std=2, demand_prob=0.8):
        ...
        planned_demand = np.zeros((self.EP_LENGTH, len(self.final_echelon_nodes)))

        for i, edge in enumerate(self.final_echelon_nodes):
    ...
    def reset(self, seed=None, options=None):
        ...
        for i, node in enumerate(self.final_echelon_nodes):
            node_idx = self.node_to_index(node)
            self.current_demand[node_idx] = self.actual_demands[0, i]
